...
type
  ...
  private
    ...
    function ReturnValue(Value: TValue; tipo: TCellsType): string; overload;
    function ReturnValue(Value: String; tipo: TCellsType): TValue; overload;
  ...
  public
    ...
    procedure UpdateGrid<T: class>(lista: TObjectList<T>);
    procedure Refresh;
    ...
    function SelectRecord(const Value, ColName: string): Boolean;
  ...
  published
    ...
    /// <remarks>
    ///   Evento Disparado Quando a edição é iniciado
    ...
    /// <remarks>
    ///   Evento Disparado durante a edição para Confirmar Alteração
    ...
    ///  Incrementar informação adicionada pelo usuário
    /// </remarks>
    {$ENDREGION}
    property OnEditEvent: TEditEvent read FOnEditEvent write SetOnEditEvent;
    {$REGION 'EndEditEvent'}
    /// <remarks>
    ///   Evento Disparado Quando a edição é finalizada
...
procedure TStringGridObject.UpdateGrid<T>(lista: TObjectList<T>);
var
  registro: TObject;
  Index, I: Integer;
  Ctx: TRttiContext;
      ...
      try
        ...
        begin
          try
            ...
            Self.Cells[I, index] := ReturnValue(PropRtti.GetValue(registro), Columns.Items[I].ColType);
          except
...
function TStringGridObject.ForceEdit: Boolean;
begin
  ...
  begin
    ...
    begin
      try
        ...
        Self.SetEditText(X, I, ReturnValue(PropRtti.GetValue(Self.Objects[0, I]), Columns.Items[X].ColType));
      except
        // campo não encontrado
      end;
    end;
...
function TStringGridObject.ReturnValue(Value: String; tipo: TCellsType): TValue;
begin
  try
    case tipo of
      ctNone:
...
function TStringGridObject.ReturnValue(Value: TValue; tipo: TCellsType): string;
begin
  try
    case tipo of
      ctNone:
...
function TStringGridObject.SelectRecord(const Value, ColName: string): Boolean;
var
...
begin
  ...
  try
    ...
    Valor := ReturnValue(Value, Self.Columns.Items[ACol].ColType);
  ...
  Cells[ACol, ARow] := ReturnValue(Valor, Self.Columns.Items[ACol].ColType);
